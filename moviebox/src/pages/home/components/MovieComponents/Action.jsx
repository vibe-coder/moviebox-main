...
function Action() {
  ...
  const url = 'https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=Action'
  const options = {
    method: 'GET',
    headers: {
  ...
  return (
    <div className='relative'>
      ...
      {modal && 
        <section  className="flex bg-transparentNeutral-2 justify-center items-center fixed z-[50] top-0 right-0 bottom-0 left-0 overflow-hidden">
            ...
            <div key={arr} className=' h-[19rem] w-[100%] md:h-[20rem] flex-col justify-between lg:h-[20rem' >
              <div className='h-[90%] w-full bg-neutral-800 flex justify-center items-center'>
                <div role="status" className="space-y-8 w-full h-full animate-pulse md:space-y-0 md:space-x-8 rtl:space-x-reverse md:flex md:items-center">
                  <div className="flex h-full w-full items-center justify-center bg-neutral-500 dark:bg-gray-800" onClick={toggleModal}>
